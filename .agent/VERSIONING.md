...
This document describes how `barista` is versioned and released. It is the
...
`barista` follows [Semantic Versioning 2.0.0](https://semver.org/): a version is
`MAJOR.MINOR.PATCH`.
...
- **Runtime reads it from `pyproject.toml`.** `barista.greet.get_version()` parses
  the `[project].version` field of `pyproject.toml` with `tomllib`, so the running
  app/CLI report the same number declared in the source of truth — no second copy
  of the string. The `barista-greet` CLI prints it via this one function.
...
3. **Full check** — runs `mise run check` (format check, lint, and tests with the
   90% coverage gate). A release is never cut from a red tree.
...
Because `barista` is **GitHub-only** (not published to PyPI), there is no registry
